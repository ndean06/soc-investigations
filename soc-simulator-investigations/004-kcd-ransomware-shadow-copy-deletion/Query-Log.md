...
| table _time host User Image CommandLine ParentImage ParentCommandLine ProcessId ParentProcessId file_name file_path Message
| sort _time
```
...
| table _time host ComputerName user User Account_Name EventCode src_ip Workstation_Name Logon_Type Authentication_Package
| sort _time
```
...
| table _time host User EventCode Image CommandLine ParentImage ParentCommandLine TargetFilename file_path file_name Hashes
| sort _time
```
...
| eval observed_path=coalesce(TargetFilename,file_path)
| table _time host User EventCode Image observed_path file_name action
| sort _time
```
...
"91.238.181.47"
| stats count min(_time) as firstSeen max(_time) as lastSeen values(user) as Usernames values(Account_Name) as Accounts values(dest_port) as Dest_Ports values(action) as Actions by host EventCode
...
| table _time host User EventCode process_name Image CommandLine ParentImage ParentCommandLine file_path TargetFilename action
| sort _time
```
...
| stats count min(_time) as firstSeen max(_time) as lastSeen values(dest_port) as dest_ports values(action) as actions by process_name Image dest_ip
| convert ctime(firstSeen) ctime(lastSeen)
| sort - count
```
